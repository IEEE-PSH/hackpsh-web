...
migrate(db, { migrationsFolder: "src/db/drizzle" })
  .then(async () => {
    ...
    await db
      .insert(schema.app_major)
      .values({ major_name: "human-centered_design_and_development" })
      .onConflictDoNothing();
    await db
      .insert(schema.app_major)
    ...
    await db
      .insert(schema.app_major)
      .values({ major_name: "nursing_(second_degree)" })
      .onConflictDoNothing();
    await db
      .insert(schema.app_major)
    ...
    await db
      .insert(schema.app_major)
      .values({
        major_name: "structural_design_&_construction_engineering_technology",
      })
      .onConflictDoNothing();
